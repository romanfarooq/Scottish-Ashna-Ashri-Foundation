...
export function SahifaDetailPage() {
  const { sahifaId } = useParams();
  const [sahifa, setSahifa] = useState(null);
  ...
  const [editSahifa, setEditSahifa] = useState({
    title: "",
    arabicTitle: "",
    subTitle: "",
    text: "",
  ...
  const fetchSahifa = useCallback(async () => {
    try {
      setLoading(true);
      const response = await fetch(
        `${API_URL}/api/v1/admin/sahifas/${sahifaId}`,
        {
          method: "GET",
          headers: {
      ...
      if (response.ok) {
        setSahifa(data.sahifa);
        setEditSahifa({
          title: data.sahifa.title,
          arabicTitle: data.sahifa.arabicTitle,
          subTitle: data.sahifa.subTitle,
          text: data.sahifa.text,
        });

        if (data.sahifa.translations?.length > 0) {
          setSelectedTranslationLanguage(data.sahifa.translations[0].language);
        }
      } else {
        toast.error(data.message);
      }
    } catch (err) {
      toast.error("Failed to fetch Sahifa.");
    } finally {
      setLoading(false);
    }
  }, [sahifaId]);
  ...
  const handleEditSahifa = async (e) => {
    e.preventDefault();
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/v1/admin/sahifas/${sahifaId}`,
        {
          ...
          credentials: "include",
          body: JSON.stringify(editSahifa),
        },
      );

      const data = await response.json();
      if (response.ok) {
        toast.success("Sahifa updated successfully");
        fetchSahifa();
      } else {
        toast.error(data.message || "Failed to update Sahifa");
      }
    } catch (error) {
  ...
  const handleEditTranslation = async (e) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/v1/admin/sahifas/${sahifaId}/translations/${selectedTranslationLanguage}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleAddTranslation = async (e) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/v1/admin/sahifas/${sahifaId}/translations`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleAudioUpload = async (event) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/v1/admin/sahifas/${sahifaId}/audio`,
        {
          method: "POST",
          credentials: "include",
          body: formData,
  ...
  const selectedTranslation = useMemo(() => {
    return sahifa?.translations?.find(
      (t) => t.language === selectedTranslationLanguage,
    );
  }, [sahifa, selectedTranslationLanguage]);
      ...
      <Card>
          ...
          <div className="flex flex-col items-center justify-between space-y-6">
            <div className="text-center">
              <CardTitle className="text-3xl font-bold text-gray-800">
                {sahifa.title} ({sahifa.arabicTitle})
                {sahifa.subTitle && (
        ...
        <CardContent className="space-y-4 bg-white p-6">
            ...
            <Dialog>
              <DialogTrigger asChild>
                ...
                >
                  ...
                  <span>Edit Sahifa</span>
                </Button>
              </DialogTrigger>
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>Edit Sahifa</DialogTitle>
                  <DialogDescription>
                    Make changes to the Sahifa details. Click save when you're
                    done.
                  </DialogDescription>
                </DialogHeader>
                <form onSubmit={handleEditSahifa} className="space-y-2">
                  <div className="space-y-1">
                    ...
                    <Input
                      id="editTitle"
                      value={editSahifa.title}
                      onChange={(e) =>
                        setEditSahifa({ ...editSahifa, title: e.target.value })
                      }
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      placeholder="Enter Sahifa sub title"
                      className="w-full"
                    />
                  </div>
                  <div className="space-y-1">
                    ...
                    <Textarea
                      id="editText"
                      value={editSahifa.text}
                      onChange={(e) =>
                        setEditSahifa({ ...editSahifa, text: e.target.value })
                      }
                      placeholder="Enter Sahifa text"
                      className="w-full"
                      rows={6}
                    />
                  </div>
          ...
          <div className="space-y-2">
            {sahifa.translations && sahifa.translations.length > 0 && (
                ...
                <div className="flex w-full items-center space-x-3">
                  ...
                  >
                    ...
                    <SelectContent className="rounded-lg bg-white shadow-md">
                      {sahifa.translations.map((translation) => (
                        <SelectItem
          ...
          <div className="mt-4 rounded-lg border border-gray-200 bg-gray-50 p-4 shadow-sm">
            ...
            {activeAudio && (
              <div className="bg-gray-50 p-4">
                <audio
                  src={`${API_URL}/api/v1/admin/sahifas/${sahifaId}/audio`}
                  controls
                  autoPlay
                  className="w-full"
                />
