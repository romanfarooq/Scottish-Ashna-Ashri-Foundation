...
export function SermonDetailPage() {
  ...
  const fetchSermon = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/v1/admin/sermons/${sermonId}`,
        {
          method: "GET",
          headers: {
      ...
      if (response.ok) {
        ...
        setEditSermon({
          ...
          arabicTitle: data.sermon.arabicTitle,
          subTitle: data.sermon.subTitle,
          text: data.sermon.text,
        });

        if (data.sermon.translations?.length > 0) {
          setSelectedTranslationLanguage(data.sermon.translations[0].language);
        }
      } else {
        toast.error(data.message);
      }
    } catch (err) {
      toast.error("Failed to fetch Sermon.");
    } finally {
      setLoading(false);
    }
  }, [sermonId]);
  ...
  const handleEditSermon = async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(data.message || "Failed to update Sermon");
      }
    } catch (error) {
  ...
  const handleEditTranslation = async (e) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/v1/admin/sermons/${sermonId}/translations/${selectedTranslationLanguage}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleAddTranslation = async (e) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/v1/admin/sermons/${sermonId}/translations`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleAudioUpload = async (event) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/v1/admin/sermons/${sermonId}/audio`,
        {
          method: "POST",
          credentials: "include",
          body: formData,
  ...
  const selectedTranslation = useMemo(() => {
    return sermon?.translations?.find(
      (t) => t.language === selectedTranslationLanguage,
    );
  }, [sermon, selectedTranslationLanguage]);
      ...
      <Card>
          ...
          <div className="flex flex-col items-center justify-between space-y-6">
            <div className="text-center">
              <CardTitle className="text-3xl font-bold text-gray-800">
                {sermon.title} ({sermon.arabicTitle})
                {sermon.subTitle && (
        ...
        <CardContent className="space-y-4 bg-white p-6">
            ...
            <Dialog>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Make changes to the Sermon details. Click save when you're
                    done.
                  </DialogDescription>
                </DialogHeader>
                <form onSubmit={handleEditSermon} className="space-y-2">
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setEditSermon({ ...editSermon, title: e.target.value })
                      }
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      placeholder="Enter Sermon sub title"
                      className="w-full"
                    />
                  </div>
                  <div className="space-y-1">
                    ...
                    <Textarea
                      id="editText"
                      value={editSermon.text}
                      onChange={(e) =>
                        setEditSermon({ ...editSermon, text: e.target.value })
                      }
          ...
          <div className="space-y-2">
            {sermon.translations && sermon.translations.length > 0 && (
                ...
                <div className="flex w-full items-center space-x-3">
                  ...
                  >
                    ...
                    <SelectContent className="rounded-lg bg-white shadow-md">
                      {sermon.translations.map((translation) => (
                        <SelectItem
          ...
          <div className="mt-4 rounded-lg border border-gray-200 bg-gray-50 p-4 shadow-sm">
            ...
            {activeAudio && (
              <div className="bg-gray-50 p-4">
                <audio
                  src={`${API_URL}/api/v1/admin/sermons/${sermonId}/audio`}
                  controls
                  autoPlay
                  className="w-full"
                />
