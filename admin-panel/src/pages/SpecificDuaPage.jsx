...
export function SpecificDuaPage() {
  ...
  const fetchSpecificDua = useCallback(async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_URL}/api/v1/admin/specific-duas`, {
        credentials: "include",
      });
      ...
      setDuas(data.specificDuas);
      setfilteredDuas(data.specificDuas);
    } catch (error) {
    ...
    if (duaToDelete) {
      try {
        const response = await fetch(
          `${API_URL}/api/v1/admin/specific-duas/${duaToDelete._id}`,
          {
            method: "DELETE",
            credentials: "include",
          },
  ...
  const navigateToDuaDetails = (dua) => {
    navigate(`/specific-dua-details/${dua._id}`);
  };
    ...
    <div className="min-h-screen bg-gray-50 p-6">
                ...
                <DialogContent className="sm:max-w-[425px]">
                  <DialogHeader>
                    ...
                    <DialogDescription className="text-gray-500">
                      Fill in the details below to add a new Specific Dua to the
                      collection.
                    </DialogDescription>
                  </DialogHeader>
      ...
      {selectedDua && (
        <EditduaDialog
          ...
          fetchSpecificDua={fetchSpecificDua}
        />
      )}
...
function EditduaDialog({ dua, selectedDua, setSelectedDua, fetchSpecificDua }) {
  const [editedDua, setEditedDua] = useState(dua);

  const handleEditDua = async () => {
    try {
      const response = await fetch(
        `${API_URL}/api/v1/admin/specific-duas/${dua._id}`,
        {
          method: "PUT",
          credentials: "include",
          headers: {
