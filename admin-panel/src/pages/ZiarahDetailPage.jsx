...
export function ZiarahDetailPage() {
  const { ziarahId } = useParams();
  const [ziarah, setZiarah] = useState(null);
  ...
  const [editZiarah, setEditZiarah] = useState({
    title: "",
    arabicTitle: "",
    subTitle: "",
    text: "",
  ...
  const fetchZiarah = useCallback(async () => {
    try {
      setLoading(true);
      const response = await fetch(
        `${API_URL}/api/v1/admin/ziarahs/${ziarahId}`,
        {
          method: "GET",
          headers: {
      ...
      if (response.ok) {
        setZiarah(data.ziarah);
        setEditZiarah({
          title: data.ziarah.title,
          arabicTitle: data.ziarah.arabicTitle,
          subTitle: data.ziarah.subTitle,
          text: data.ziarah.text,
        });

        if (data.ziarah.translations?.length > 0) {
          setSelectedTranslationLanguage(data.ziarah.translations[0].language);
        }
      } else {
        toast.error(data.message);
      }
    } catch (err) {
      toast.error("Failed to fetch Ziarah.");
    } finally {
      setLoading(false);
    }
  }, [ziarahId]);
  ...
  const handleEditZiarah = async (e) => {
    e.preventDefault();
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/v1/admin/ziarahs/${ziarahId}`,
        {
          ...
          credentials: "include",
          body: JSON.stringify(editZiarah),
        },
      );

      const data = await response.json();
      if (response.ok) {
        toast.success("Ziarah updated successfully");
        fetchZiarah();
      } else {
        toast.error(data.message || "Failed to update Ziarah");
      }
    } catch (error) {
  ...
  const handleEditTranslation = async (e) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/v1/admin/ziarahs/${ziarahId}/translations/${selectedTranslationLanguage}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleAddTranslation = async (e) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/v1/admin/ziarahs/${ziarahId}/translations`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleAudioUpload = async (event) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/v1/admin/ziarahs/${ziarahId}/audio`,
        {
          method: "POST",
          credentials: "include",
          body: formData,
  ...
  const selectedTranslation = useMemo(() => {
    return ziarah?.translations?.find(
      (t) => t.language === selectedTranslationLanguage,
    );
  }, [ziarah, selectedTranslationLanguage]);
      ...
      <Card>
          ...
          <div className="flex flex-col items-center justify-between space-y-6">
            <div className="text-center">
              <CardTitle className="text-3xl font-bold text-gray-800">
                {ziarah.title} ({ziarah.arabicTitle})
                {ziarah.subTitle && (
        ...
        <CardContent className="space-y-4 bg-white p-6">
            ...
            <Dialog>
              <DialogTrigger asChild>
                ...
                >
                  ...
                  <span>Edit Ziarah</span>
                </Button>
              </DialogTrigger>
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>Edit Ziarah</DialogTitle>
                  <DialogDescription>
                    Make changes to the Ziarah details. Click save when you're
                    done.
                  </DialogDescription>
                </DialogHeader>
                <form onSubmit={handleEditZiarah} className="space-y-2">
                  <div className="space-y-1">
                    ...
                    <Input
                      id="editTitle"
                      value={editZiarah.title}
                      onChange={(e) =>
                        setEditZiarah({ ...editZiarah, title: e.target.value })
                      }
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      placeholder="Enter Ziarah sub title"
                      className="w-full"
                    />
                  </div>
                  <div className="space-y-1">
                    ...
                    <Textarea
                      id="editText"
                      value={editZiarah.text}
                      onChange={(e) =>
                        setEditZiarah({ ...editZiarah, text: e.target.value })
                      }
                      placeholder="Enter Ziarah text"
                      className="w-full"
                      rows={6}
                    />
                  </div>
          ...
          <div className="space-y-2">
            {ziarah.translations && ziarah.translations.length > 0 && (
                ...
                <div className="flex w-full items-center space-x-3">
                  ...
                  >
                    ...
                    <SelectContent className="rounded-lg bg-white shadow-md">
                      {ziarah.translations.map((translation) => (
                        <SelectItem
          ...
          <div className="mt-4 rounded-lg border border-gray-200 bg-gray-50 p-4 shadow-sm">
            ...
            {activeAudio && (
              <div className="bg-gray-50 p-4">
                <audio
                  src={`${API_URL}/api/v1/admin/ziarahs/${ziarahId}/audio`}
                  controls
                  autoPlay
                  className="w-full"
                />
